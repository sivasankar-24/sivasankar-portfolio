...
# Read API key from environment variable (make sure this matches your Railway env variable exactly)
OPENROUTER_API_KEY = os.getenv("sivasankar-portfolio-api-key")
...
def chat(query: QueryRequest):
    ...
    user_msg = {"role": "user", "content": str(query.message)}  # Ensure content is string
    ...
    # Debug log to trace request body
    print(f"Sending to OpenRouter API: {json.dumps(body)}")

    response = requests.post("https://openrouter.ai/api/v1/chat/completions", headers=headers, json=body)
    print(f"Response status: {response.status_code}, body: {response.text}")  # Debug log
